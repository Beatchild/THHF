...
import { Bold, Italic, Underline as UnderlineIcon, Heading2, Heading3, List, Quote, ImageIcon, Strikethrough, Code, Link as LinkIcon, Video as YoutubeIcon, Music, Highlighter, Images, AlignLeft, AlignCenter, AlignRight, AlignJustify, Minus, Undo, Redo, Eraser } from 'lucide-react'
...
export default function TiptapEditor({ content, onChange }: { content: string, onChange: (c: string) => void }) {
  const editor = useEditor({
    extensions: [
      ...
      Image.configure({ inline: true, HTMLAttributes: { class: 'rounded-xl shadow-2xl mx-auto my-12' } }),
...
fs.writeFileSync('src/components/admin/TiptapEditor.tsx', editorCode, 'utf8');

const cssAppend = `
/* Modern Tiptap Placeholder */
...
fs.appendFileSync('src/app/globals.css', cssAppend, 'utf8');
